...
import { ORIGIN, bank, batchId, join, kill, open, press, table } from './support.js'
...
describe('a session id is not an address', () => {
  test('a made-up session id buys nothing', async () => {
    ...
    for (const forged of ['', 'session', 'A'.repeat(64), null, 42, {}]) {
      await expect(game.bank(forged, ORIGIN, batchId())).rejects.toThrow(SessionError)
    }
  }, 20_000)
  ...
  test('a session used from another origin is refused', async () => {
    ...
    await expect(bank(game, session, batchId(), 'https://evil.example')).rejects.toThrow('different origin')
  }, 20_000)
  ...
  test('one session cannot bank another session’s presses', async () => {
    ...
    await expect(bank(game, mine)).rejects.toThrow('has not seen any presses from you yet')
  }, 30_000)
})
